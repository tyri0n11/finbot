...
from models.simple_weatherapi import (
    SimpleCurrentResponse, 
...
class SimpleWeatherAPIService:
    def __init__(self):
        self.api_key = os.getenv("WEATHERAPI_KEY", "")
        self.base_url = "http://api.weatherapi.com/v1"
        
    async def get_current_weather(self, location: str) -> Optional[SimpleCurrentResponse]:
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                data = response.json()
                return SimpleCurrentResponse(**data)
            except Exception as e:
                print(f"Error getting current weather: {e}")
                return None
    
    async def get_forecast_weather(self, location: str, days: int = 1) -> Optional[SimpleForecastResponse]:
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                response.raise_for_status()
                data = response.json()
                return SimpleForecastResponse(**data)
            except Exception as e:
                print(f"Error getting forecast weather: {e}")
                return None
    
    def _convert_to_weather_create(self, api_data: SimpleCurrentResponse) -> WeatherCreate:
    ...
    def _convert_forecast_to_weather_create(self, api_data: SimpleForecastResponse, day_data) -> WeatherCreate:
...
simple_weatherapi_service = SimpleWeatherAPIService()
